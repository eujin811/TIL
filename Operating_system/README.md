...
**운영체제의 종류**
1. 일괄 처리 시스템
	- 컴퓨터의 작업 준비 시간 동안 CPU가 유휴상태로 작업을 기다릴 경우 이용 효율이 떨어져 작업간의 자동화시켜주는 작업 제어 카드개념 도입하여 해결하였다. 이를 초창기 운영체제의 상주모니터(resident monitor)라고 하며 초창기 일괄 처리(batch process) 시스템의 효시
	- 상주모니터(resident monitor)에 의한 일괄처리 시스템
		1. 프로그래머는 자동 작업 순서 작업 제어 카드에 기록
		2. 상주 모니터는 작업 제어 카드에 지시한 대로 자동 작업 순서 제공 받는다. 상주 모니터는 제어 카드 해석기를 통해 작업 제어 카드 내용 해석
		3.  상주 모니터는 해당 사용자 프로그램을 기억장치에 적재하고 사용자 모드로 전환
		4. 프로그램 수행이 끝나면 제어는 상주 모니터 모드로 복귀
		5. 2,3,4 반복
		- 유휴 시간 크게 줄지 않음.
	- 단점 
		- 시스템 효율성 떨어짐
		- 작업 시행시 사용자와 작업간의 대화 부족으로 일정 시간 경과 후 결과 출력되 작업 완료 사이의 반환시간이 늦음
		- 작업수행 중 대화 불가로 처리 중간에 발생할 수 있는 모든 상황에 적절하게 대처할 관리기능 제공 필요
		- 프로그램 오류 수정 시 당시 상태 그대로 출력해 정적으로 수행
		- 하나의 작업이 수행되면 작업이 시스템의 모든 자원 독점해 CPU 이용률 저하된다. CPU와 입출력 장치의 속도 차이로 입출력이 완료되는 동안 CPU 유휴 상태

2. 온라인 vs 오프라인 시스템
	- ** 온라인 시스템**
		- 대화식 시스템(interactive system)은 CPU와 주변 장치가 온라인으로 연결되어 작동하고 사용자와 컴퓨터 시스템 간에 온라인 통신 제공해 사용자가 운영체제나 프로그램에 직접 명령을 주고 즉시 응답을 받을 수 있는 시스템
		- 특징
			- 사용자는 프로그램을 쉽게 실험 가능
			- 실행 결과를 곧바로 받아 응답시간 짧음
			- 바로 실행 결과가 출력되어 프로그래머의 검사 개발에 유효성 제공
			- CPU는 프로그래머나 조작원이 취할 동작을 기다리는데 유휴 시간 생김
	- ** 오프라인 시스템**
		- CPU 이용률 높이기 위해 제작됨
		- 카드 판독기와 라인 프린터와 CPU 중간에 자기 테이프나 자기 디스크로 대체하여 자료나 프로그램 입력 시 CPU가 직접 카드를 읽지 않고 카드에 있는 내용을 자기 테이프에 복사해 테이프가 차게 되면, 자기 테이프를 컴퓨터로 옮겨 프로그램이 자기 테이프로부터 내용을 읽어 들인다. 출력 시 자기 테이프상 먼저 기록 후 테이프 내용 출력 (key-to-tape)
		- 특징
			- CPU가 키드 판독기와 라인 프린터의 속도에 영향 받지 않고, 훨씬 빠른 자기 테이프의 속도에 영향
			- 온라인 시스템에서의 응용 프로그램이 오프라인 입출력 동작으로 전환하는데 어떠한 수정도 필요하지 않음.
			- 하나의 프로그램이 다른 입출력 장치와 함께 실행될 수 있는 장치 독립성 보장. 프로그램이 논리적 입출력 장치를 사용해 작성하고 운영체제는 이에 맞는 물리적 장치를 결정한다.
			- 하나의 CPU에 대해 여러 대의 카드 판독기와 라인 프린터 및 자기 테이프를 사용할 수 있음.
		
3. 다중 프로그래밍 시스템
	- CPU가 항상 이용되게 주기억 장치에 여러 개의 프로그램을 적재함으로써 CPU의 이용률을 극대화하고자 제작됨
	- 주기억 장치에 여러 개의 프로그램을 적재하여 처리할 때, 한 프로그램이 입출력 작업을 하는 동안, 다른 프로그램은 CPU를 이용해 연산처리 작업을 수행하는 과정 반복
